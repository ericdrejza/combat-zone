  ...
  return (
    <section className="p-5" aria-labelledby="settings-data-heading">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button className="flex items-center gap-2 rounded-xl border border-canvas-line bg-white px-4 py-2 text-sm font-medium transition hover:bg-canvas" onClick={onExportWorkspace} type="button"><Download aria-hidden="true" className="h-4 w-4" />Export workspace</button>
        <label className={`flex items-center gap-2 rounded-xl border border-canvas-line bg-white px-4 py-2 text-sm font-medium transition ${readOnly ? "cursor-not-allowed opacity-50" : "cursor-pointer hover:bg-canvas"}`}>
      ...
      <div className="mt-4 rounded-2xl border border-red-300 bg-red-50 p-4">
        ...
        {!resetOpen ? (
          ...
          <div className="mt-3">
            ...
            <button className="mt-3 flex items-center gap-2 rounded-xl border border-canvas-line bg-white px-3 py-2 text-sm font-medium transition hover:bg-canvas" onClick={onExportWorkspace} type="button"><Download aria-hidden="true" className="h-4 w-4" />Export workspace first</button>
            <label className="mt-4 block text-sm font-medium text-red-950">Type {RESET_CONFIRMATION} to continue<input aria-label="Local reset confirmation" autoComplete="off" className="mt-1 w-full rounded-xl border border-red-300 bg-white px-3 py-2 font-mono text-sm outline-none focus:border-red-700" onChange={(event) => setConfirmation(event.target.value)} value={confirmation} /></label>
            {error ? <p className="mt-2 text-sm text-red-800" role="alert">{error}</p> : null}
            <div className="mt-4 flex justify-end gap-2">
              <button className="rounded-xl border border-canvas-line bg-white px-4 py-2 text-sm font-medium" disabled={resetting} onClick={() => { setResetOpen(false); setConfirmation(""); }} type="button">Cancel</button>
