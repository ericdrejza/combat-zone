...
const MANIFEST_KEY = "workspace";
const RECOVERY_KEY = "draft";
...
export class IndexedDbWorkspaceRepository implements WorkspaceRepository {
  ...
  async createEncounter(state: EncounterState, folderId: string | null = null): Promise<EncounterRecord> {
    ...
    await this.write(STORE_ENCOUNTERS, async (transaction) => { transaction.objectStore(STORE_ENCOUNTERS).add(record); return undefined; });
    return clone(record);
  }
  ...
  async saveEncounter(state: EncounterState, options: RepositorySaveOptions = {}): Promise<EncounterRecord> {
    assertEncounterState(state);
    const database = await this.database();
    const transaction = database.transaction(STORE_ENCOUNTERS, "readwrite");
  ...
  async moveEncounter(id: string, folderId: string | null): Promise<EncounterRecord> {
    ...
    await this.write(STORE_ENCOUNTERS, async (transaction) => { transaction.objectStore(STORE_ENCOUNTERS).put(record); return undefined; });
    return clone(record);
  }

  async deleteEncounter(id: string): Promise<void> {
    const manifest = await this.getManifest();
    await this.write([STORE_ENCOUNTERS, STORE_MANIFEST], async (transaction) => {
  ...
  async saveRecoveryDraft(state: EncounterState): Promise<RecoveryDraftRecord> {
    ...
    await this.write(STORE_RECOVERY, async (transaction) => { transaction.objectStore(STORE_RECOVERY).put(record, RECOVERY_KEY); return undefined; });
    return clone(record);
  }

  async deleteRecoveryDraft(): Promise<void> { await this.write(STORE_RECOVERY, async (transaction) => { transaction.objectStore(STORE_RECOVERY).delete(RECOVERY_KEY); return undefined; }); }
  ...
  async saveLibrary(state: LibraryState): Promise<LibraryRecord> {
    ...
    await this.write(STORE_LIBRARY, async (transaction) => { transaction.objectStore(STORE_LIBRARY).put(record, LIBRARY_KEY); return undefined; });
    return clone(record);
  }
  ...
  async importWorkspace(envelope: WorkspaceExportEnvelope, mode: "overwrite" | "merge"): Promise<WorkspaceManifest> {
    ...
    await this.write([STORE_MANIFEST, STORE_ENCOUNTERS, STORE_RECOVERY, STORE_LIBRARY], async (transaction) => {
