...
function getActorSplitSectionId(layoutGroup: string): string {
  return layoutGroup === 'ally' ? 'hero' : layoutGroup;
}
...
export function getEngagementAwareSplitInput(state: EncounterState, zoneId: string) {
  ...
  const actors = state.actors.allIds.flatMap((actorId) => {
    ...
    return actor?.currentZoneId === zoneId ? [{ ...toNestingActor(actor), splitSectionId: engagementSectionByActor.get(actor.id) ?? getActorSplitSectionId(actor.layoutGroup) }] : [];
  });
